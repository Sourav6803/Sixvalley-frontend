...
const FlashDealAddProduct = () => {
    ...
    useEffect(() => {
        axios.get(`${server}/flashDeal/get-flashDeal/${id}`, { withCredentials: true })
            .then(res => {
                setIsLoading(false);
                setSelectedDeal(res.data.flashDeal);
            })
            .catch(error => {
                setIsLoading(false);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axios.put(`${server}/flashDeal/update-flashDeal/${id}`, {
    ...
    const handleDelete = useCallback(async () => {
        setIsSubmitting(true);
        try {
            const res = await axios.put(`${server}/flashDeal/remove-product/${id}/${productId}`, {}, { withCredentials: true });
    ...
    return (
        <div className='w-full 800px:p-5 p-2 bg-gray-200'>
            ...
            <Modal
                ...
                message="Are you sure you want to delete this product from flash deal?"
                isDelete={isDelete}
                buttonText={'Delete'}
            />
        </div>
...
export default FlashDealAddProduct;
