...
const SubCategory = () => {
    ...
    const { success, error, allSubCategory } = useSelector((state) => state?.subCategory);
    ...
    const handleSubmit = useCallback((e) => {
        ...
        try {
            dispatch(createSubCategory({ name, priority, mainCategory }))
    ...
    }, [name, priority, mainCategory, dispatch]);


    const handleDelete = useCallback(async (id) => {
        try {
            setIsDelete(true)
            const res = await axios.delete(`${server}/subCategory/delete-subCategory/${id}`, { withCredentials: true });
            setIsDelete(false)
            toast.success(res.data.message || "Sub Category Deleted");
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        const filterCategory = allSubCategory.filter((catg) =>
            catg.name.toLowerCase().includes(searchTearm.toLowerCase())
        );
        setSearchData(filterCategory);
    };
                    ...
                    isSubmitting ? (<div className='w-full h-full flex items-start justify-center'><Loader /></div>) : (
                        ...
                        <div className="p-4 rounded-md ">
                            <form className='items-center justify-around grid grid-cols-1 lg:grid-cols-3 gap-2 '>
                                <div className="mb-4 ">
                                    <label htmlFor="name" className="block text-lg font-medium text-gray-700">Sub Category Name *</label>
                                    <input
                                        type="text"
                                        name="name"
                                        autoComplete="name"
                                    ...
                                    <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                                                {(searchData || allSubCategory)?.length > 0 ? (searchData || allSubCategory).map((cat, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                            <div className='flex items-center justify-end'>
                                                                <div className='w-full mt-4 flex items-center justify-center gap-2 bg-white  '>
                                                                    ...
                                                                    <Link to={`/admin/dashboard/subCategory/${cat?._id}`}>
