...
const Header = ({ activeHeading, searchContentOpen, setSearchContentOpen }) => {
  ...
  //   setIsLoading(true);
  //   try {
  //     const response = await axios.get(`${server}/api/search`, {
  //       params: {
  //         query: searchQuery,
  //         suggestOnly: true,
  //       },
  ...
  // Perform the main search when the user presses Enter or submits the form
  ...
  const fetchSuggestions = async (searchQuery) => {
    try {
      const response = await axios.get(
        `${server}/search/suggest?query=${searchQuery}`
      );
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      await axios.post(`${server}/activity/logActivity`, {
        userId: userId, // Replace with actual user ID from your app context
        type: "search",
        searchTerm: query,
      });
      ...
      // Perform the search
      const response = await axios.get(
        `${server}/search/search-product?query=${query}`
      );
      setResults(response.data.results);
    } catch (error) {
    ...
    <>
      <div className={`${styles.section} `}>
        ...
        >
          ...
          {/* search box */}
          <div id="search" className="w-[50%] relative">
            <AiOutlineSearch
              size={30}
            ...
            />
            {/* <input
              className="h-[40px] w-full px-10 border-[#3957db] border-[2px] rounded-md"
              type="text"
              placeholder="Search product.."
              onClick={()=> !setSearchContentOpen(true)}
              value={searchTearm}
      ...
      <div
        ...
        } transition hidden 800px:flex z-50 top-0 left-0 items-center justify-between w-full  h-[70px]`}
        style={{
          background:
          ...
          <div className="flex">
            ...
            <div className={`${styles.noramlFlex}`}>
              ...
              >
                ...
                {isAuthenticated && wishlist?.length >= 1 && (
      ...
      <div
        className={` ${
          active === true ? "shadow-sm fixed top-0 left-0 z-10" : null
        } w-full h-[60px] bg-blue-600  z-50 top-0 left-0 shadow-sm 800px:hidden md:flex-row`}
      >
        <div className="w-full flex items-center justify-between ">
          ...
          <div className=" flex items-center justify-center ">
            ...
            <div>
              <div className="relative " onClick={() => setOpenWishlist(true)}>
                ...
                {isAuthenticated && wishlist?.length > 0 && (
                  <span className="absolute right-0 top-0 rounded-full bg-[#ffffff] w-3 h-3 top right p-0 m-0  font-mono text-[10px] leading-tight text-center">
                    {wishlist && wishlist?.length}
                  </span>
                )}
              </div>
            ...
            >
              ...
              />
              {isAuthenticated && cart?.length >= 1 && (
                <span className="absolute right-1 top-0 rounded-full bg-white w-3 h-3 top  p-0 m-0  font-mono text-[10px]  leading-tight text-center">
                  {cart && cart?.length}
                </span>
              )}
            </div>
        ...
        {/* <div
          className={`fixed bg-white top-[60px] left-0 z-50 max-w-md mx-auto rounded-lg h-auto overflow-visible md:max-w-xl w-full shadow-lg md:hidden`}
        >
          <div className="md:flex">
            <div className="w-full p-1">
              ...
              {searchTearm && searchData?.length > 0 && (
                <div className="absolute top-full bg-slate-100 shadow max-h-60 w-full z-200 left-0 p-3 overflow-y-auto">
                  {searchData.map((i, index) => (
                    <Link to={`/product/${i?._id}`} key={index}>
                      <div
                        className="w-full flex items-start py-3 "
                        onClick={() => handleSearch(i?._id, i?.name)}
                      >
                        <img
                          src={i?.images[0]?.url}
                          alt="img"
        ...
        {open && (
          <div
            id="screen"
            onClick={handleClose}
            className={` fixed w-full bg-[#0000005f] z-20 h-full top-0  `}
          >
            <div className="fixed w-[60%] bg-[#fff] h-screen top-0 left-0 z-10 overflow-y-scroll ">
              <div className="w-full justify-between flex pr-3 bg-blue-500 mb-2">
                ...
                <RxCross1
                  size={30}
                  className="mt-3 text-white ml-4 border-2 rounded-md border-white hover:border-red-500 cursor-pointer"
              ...
              <ul className="pt-2">
                {ProfileMenu.map((menu, index) => (
                  <React.Fragment key={index}>
                    ...
                    >
                      <span className="text-xl block float-left hover:scale-110">
                        {menu.icon}
                      </span>
                      <Link
                        to={menu?.link}
                        className={`text-[14px] font-[500] flex-1 ${
                          !open && "hidden"
                        }`}
                      >
                        {menu.title}
              ...
              <button
                type="button"
                className="text-white bg-gradient-to-br mt-3 ml-4 from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
              >
                <Link to={`${isSeller ? "/dashboard" : "/shop-welcome"}`}>
                  <h1 className="text-[#fff] flex items-center ">
              ...
              {isAdmin && (
                <button
                  type="button"
                  className="text-white bg-gradient-to-br ml-4 from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
                >
              ...
              <div className="mt-5 ml-4">
                <h2 className="text-xl  text-slate-700 font-semibold mb-2 ">
                  Follow Us
                </h2>
      ...
      {searchOpen && (
        <div
          className={`max-w-md mx-auto rounded-lg overflow-hidden md:max-w-xl shadow-lg md:hidden`}
        >
          <div className="md:flex">
            <div className="w-full p-1">
              <div className="relative flex items-center justify-between bg-white rounded-lg shadow-sm border-2">
