...
const ReadyToShipOrderTable = ({ confirmedOrder, isLoading }) => {
  ...
  useEffect(() => {
    if (searchTearm) {
      const filterProduct = confirmedOrder?.filter((order) =>
  ...
  const currentData = (
    filterOrders !== null ? filterOrders : confirmedOrder
  ...
  const totalPages = Math.ceil(
    (confirmedOrder || filterOrders)?.length / itemsPerPage
  );
        ...
        <div className="w-full  bg-slate-50">
                                      ...
                                      <div className="flex flex-col gap-2">
                                        <button
                                          onClick={() => {
                                            ...
                                            setInoviceData(order);
                                            setOrderId(order?._id);
                                          }}
                                          disabled={isDisabled}
                                          className={`px-3 py-1 rounded-md shadow-md flex items-center justify-center ${
                                            isDisabled
          ...
          {approvedModalOpen && (
            <Modal
              ...
              onConfirm={() => {
                downloadInvoice(orderId, invoiceData);
                setTimeout(() => {
                  setApprovedModalOpen(false);
                }, 500);
              }}
              title="Want to Download this Order Label ?"
              buttonText={"Download Label"}
              message="After download you need to print this label."
            />
          )}
        </div>
      )}
