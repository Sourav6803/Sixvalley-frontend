...
const PendingProduct = () => {
  ...
  const pendingProducts = products && products?.filter((product) => product.approved === "Pending")
  ...
  const currentData = (searchData || pendingProducts)?.slice(indexOfFirstItem, indexOfLastItem);

  // Calculate total pages
  const totalPages = Math.ceil((searchData || pendingProducts)?.length / itemsPerPage);
      ...
      {isLoading === true ? (
        ...
        <div className='w-full p-2 md:p-5 bg-gray-200'>
          <div className='flex items-center gap-2'>
            ...
            <h3 className="text-[20px] text-slate-600 font-Poppins font-semibold">Pending Product List: {products?.length}</h3>
          </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          ...
                          <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                            {currentData?.length > 0 ? currentData?.map((product, index) => (
                              ...
                              <tr>
                                <td colSpan="5" className="text-center py-4 text-gray-500 dark:text-gray-400">No Pending product found</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
