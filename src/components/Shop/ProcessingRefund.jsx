...
const ProcessingRefund = () => {
  ...
  const processingRfnd = orders?.filter(order => order?.status === "Processing refund")
  ...
  useEffect(() => {
    if (searchTearm) {
      const filterProduct = processingRfnd?.filter((order) =>
  ...
  const currentData = (searchData || processingRfnd)?.slice(indexOfFirstItem, indexOfLastItem);

  // Calculate total pages
  const totalPages = Math.ceil((searchData || processingRfnd)?.length / itemsPerPage);
      ...
      {isLoading ? (
        ...
        <div className='w-full p-2 md:p-5 bg-gray-200'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <h3 className="text-[20px] text-slate-600 font-Poppins font-semibold">Processing Refund Order List: {processingRfnd?.length}</h3>
          </div>
            ...
            <div className='flex items-center justify-between  flex-col md:flex-row gap-2 md:gap-0'>
              <div className='flex items-center text-white   '>
                <div className='bg-blue-700 rounded-md px-3 py-1 flex items-center justify-center gap-1 cursor-pointer' onClick={() => navigate("/dashboard-create-product")}>
                  ...
                  <div className=''>Order List : {processingRfnd?.length}</div>
                </div>
              </div>
                                ...
                                <td className="px-4 py-4 text-sm whitespace-nowrap">
                                  <div className='flex items-center justify-end'>
                                    <div className='w-full mt-4 flex items-center justify-center gap-2 bg-white  '>
                                      ...
                                      >
                                        <AiOutlineEye className="text-blue-500 hover:text-white" onClick={() => navigate(`/seller/refund/order/${order?._id}`)} />
                                      </button>
...
export default ProcessingRefund;
