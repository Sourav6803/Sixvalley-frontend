...
const CancledOrder = () => {
    ...
    const cancledOrders = orders?.filter(order => order?.status === "Canceled")
    ...
    useEffect(() => {
        if (searchTearm) {
            const filterProduct = cancledOrders?.filter((order) =>
    ...
    const currentData = (searchData || cancledOrders)?.slice(indexOfFirstItem, indexOfLastItem);

    // Calculate total pages
    const totalPages = Math.ceil((searchData || cancledOrders)?.length / itemsPerPage);
            ...
            {isLoading ? (
                ...
                <div className='w-full p-2 md:p-5 bg-gray-200'>
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        <h3 className="text-[20px] text-slate-600 font-Poppins font-semibold">Cancled Order List: {orders?.length}</h3>
                    </div>
                        ...
                        <div className='flex items-center justify-between  flex-col md:flex-row gap-2 md:gap-0'>
                            <div className='flex items-center text-white   '>
                                <div className='bg-blue-700 rounded-md px-3 py-1 flex items-center justify-center gap-1 cursor-pointer' onClick={() => navigate("/dashboard-create-product")}>
                                    ...
                                    <div className=''>Cancled Order List : {orders?.length}</div>
                                </div>
                            </div>
                                                ...
                                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                    ...
                                                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                                                        {currentData?.length > 0 ? currentData?.map((order, index) => (
                                                            ...
                                                            <tr>
                                                                <td colSpan="5" className="text-center py-4 text-gray-500 dark:text-gray-400">No cancled orders found</td>
                                                            </tr>
                                                        )}
                                                    </tbody>
                                                </table>
