...
import UploadTips from "./Product/UploadTips";

const ENDPOINT = "http://localhost:4000";
const socketId = socketIO(ENDPOINT, { transports: ["websocket"] });

const CreateProduct = ({ sidebarOpen }) => {
  ...
  const modules = {
    toolbar: [
      [{ header: [1, 2, 3, false] }], // Add header options
  ...
  const handleAddAttributeValue = (attribute) => {
    ...
    const newValues = {
      ...
      [attribute]: "",
      [attribute + "List"]: [
        ...(attributeValues[attribute + "List"] || []),
  ...
  const updateVariations = (attributes, attributeValues) => {
    ...
    const generateCombinations = (lists, prefix = []) => {
      ...
      lists[0].forEach((item) =>
        generateCombinations(lists.slice(1), [...prefix, item])
      );
    };
  ...
  const handleVariationChange = (index, field, value) => {
    ...
    if (field === "originalPrice" || field === "discountAmount") {
      ...
      updatedVariations[index][updatedVariations[index].length - 1][field] =
  ...
  const getVariationKey = useCallback(
    (variation) => {
      ...
      prevVariations.map((variation) => {
        if (getVariationKey(variation) === variationKey) {
          ...
          updatedVariation[lastIndex] = {
            ...updatedVariation[lastIndex],
            images: [
              ...(updatedVariation[lastIndex].images || []),
              ...fileURLs,
            ],
          };
  ...
  const handleCategoryChange = useCallback(
    (e) => {
      ...
      const filteredSubCategories = allSubCategory?.filter(
        (subCat) => subCat.mainCategory === selectedMainCategory
      );
      setFilteredSubCategories(filteredSubCategories);
      setFilterBrand(
        allBrand?.filter((brand) => brand?.category === selectedMainCategory)
      );
  ...
  const handleGetQuillContent = () => {
    if (quillRef.current) {
      ...
      const contents = quill.getContents(); // Get the current editor contents as Delta object
      console.log(contents); // Logs Delta format
  ...
  const handleSubCategoryChange = useCallback(
    (e) => {
      ...
      const filteredSubCategories = allSubSubCategory.filter(
        (subCat) => subCat.subCategory.trim() === selectedSubCategory.trim()
      );
  ...
  useEffect(() => {
    ...
    const newVariations = variations.map((variation, index) => {
      ...
      const { originalPrice, discountAmount, discountType } =
        variation[lastIndex];
  ...
  const calculateAfterDiscountPrice = (
    originalPrice,
    discountAmount,
    discountType
  ) => {
    ...
    if (discountType === "Flat") {
      ...
      afterDiscountPrice =
        originalPriceNum -
        Math.ceil((discountAmountNum / 100) * originalPriceNum);
    }
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      const [uploadedMainImages, variantsData] = await Promise.all([
        (async () => {
          try {
            ...
            const mainImageUploadResponse = await axios.post(
          ...
          try {
            ...
            return await Promise.all(
              variations.map(async (variation) => {
                ...
                const variantImageUploadResponse = await axios.post(
                  `${server}/product/uploadImages`,
                  variantImagesForm,
                  {
                    headers: { "Content-Type": "multipart/form-data" },
                ...
                return {
                  ...
                  discountAmount:
                    variation[variation.length - 1].discountAmount,
                  afterDiscountPrice:
                    variation[variation.length - 1].afterDiscountPrice,
      ...
      try {
        ...
        newForm.append("dimension", dimensions);
      ...
      <div className="w-full px-1 md:px-3 bg-gray-200">
        ...
        <div className="w-full mt-2 bg-white p-3  rounded-md hover:shadow-md">
          <div className="flex items-center gap-2 mt-2 rounded-md bg-slate-200 p-3">
            ...
            <h3 className="text-[20px] text-slate-600 font-Poppins font-medium">
              General Setup
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-4 mt-3  gap-3">
            ...
            <div>
              ...
              <select
                className="w-full text-slate-500 mt-2 border h-[35px] rounded-[5px]"
                value={subCategory}
              ...
              <div className="relative w-full mt-2">
                ...
                >
                  <span className="text-sm sm:text-base truncate">
                    {subSubCategory || "Choose a Sub Sub-category"}
                  </span>
                  <span className="ml-2">&#9662;</span>
                </div>
                  ...
                  <div className="absolute w-full mt-2 border rounded-[5px] bg-white z-10">
                    {filteredSubSubCategories?.map((category) => (
                      <div
                        ...
                        className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-gray-100 text-gray-600"
                        onClick={() =>
                          handleSubSubCategoryChange(category?.name)
                        }
                      >
                        <img
                          src={category.image.url}
            ...
            <div>
              <label
                className="pb-2 text-slate-700 font-medium text-md"
                htmlFor="type"
              >
                Product Type
              </label>
            ...
            <div className="relative">
              <div className="flex items-center justify-between">
                <label className=" text-slate-700 font-medium text-md">
                  Search Tags
                </label>
            ...
            <div>
              <label className="pb-2">
                Max Puchase Limit <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
            ...
            <div className="">
              ...
              <div className="pb-2 text-slate-700 font-medium text-md">
                <select
            ...
            <div className="">
              ...
              <div
                className="cursor-pointer bg-gray-100 border mt-2 border-gray-300 rounded-md p-1.5 flex items-center justify-between"
                onClick={() => setIsDimensionOpen(!isDimensinonOpen)}
              >
                <span className="text-slate-700 font-medium">
                  Package Dimensions
                </span>
                <span>{isDimensinonOpen ? "▲" : "▼"}</span>
              </div>

              {/* Collapsible Section */}
              {isDimensinonOpen && (
                <div className="mt-2 border border-gray-300 rounded-md p-3 bg-white shadow-sm">
                  {/* Length Input */}
                  <div className="flex items-center mb-2">
                    <label className="w-1/3 text-slate-600">Length</label>
                    <div className="flex items-center w-2/3">
                      ...
                      />
                      <span className="ml-2 text-slate-500">cm</span>
                    </div>
                  </div>

                  {/* Width Input */}
                  <div className="flex items-center mb-2">
                    <label className="w-1/3 text-slate-600">Width</label>
                    <div className="flex items-center w-2/3">
                      <input
                        type="number"
                        name="width"
                  ...
                  <div className="flex items-center">
                    <label className="w-1/3 text-slate-600">Height</label>
                    <div className="flex items-center w-2/3">
                      <input
                        type="number"
                        name="height"
            ...
            <div className="">
              ...
              <div className="border border-gray-300 rounded-[3px] mt-2">
                ...
                >
                  <option value="No warrentye">No warrenty</option>
        ...
        <div className="w-full mt-2 bg-white p-3  rounded-md hover:shadow-md">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-4 mt-3  gap-3">
            ...
            <div>
              ...
              >
                <option
                  className="mt-2 appearance-none block w-full px-3 h-[35px] border border-gray-300 rounded-[3px] text-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                  value=""
                >
                  Choose an type
                </option>
            ...
            <div>
              ...
              <input
                ...
                name="price"
                min={0}
                defaultValue={afterDiscountPrice}
                className="mt-2 appearance-none block w-full px-3 h-[35px] border border-gray-300 rounded-[3px] placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                readOnly
                placeholder="Ex: 300"
              />
              ...
              <div key={attribute.value} className="mb-2">
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) =>
                      handleAttributeValueChange(e, attribute.value)
                    }
                        ...
                        <td className="px-4  whitespace-nowrap text-gray-500">
                          <div className="flex items-center gap-2  rounded-md p-2">
                            <input
                              type="file"
                              onChange={(e) =>
                                handleImageUpload(e, getVariationKey(variation))
                              }
                              className="hidden"
                              ref={fileInputRef}
                            />
                            ...
                            <div className="flex gap-2 overflow-x-auto">
                              {colorImages[getVariationKey(variation)]?.map(
                                (url, imgIndex) => (
                                  <div
                                    key={imgIndex}
                                    className="relative w-[60px] h-[50px]"
                                  >
                                    <img
                                      src={url}
                                      alt={`Color ${
                                        variation[selectedAttributes.length]
                                      } - ${imgIndex}`}
              ...
              {images &&
                images.map((i) => (
                  <img
                    ...
                    key={i}
                    alt=""
                    className="h-[150px] w-[130px] object-cover m-2 border-2"
                  />
                ))}
            </div>
          </div>
