...
const FashionPage = () => {
  ...
  const mainbanner = allBanner?.filter(
    (banner) =>
      banner?.bannerType === "Main Banner" && banner?.resourceType === "Fashion"
  );
  ...
  useEffect(() => {
    const filterData = allProducts?.filter(
      (product) => product?.category.trim() === "Fashion"
    );
  ...
  useEffect(() => {
    const filterData =
      allSubSubCategory &&
      allSubSubCategory?.filter((cat) => cat.mainCategory.trim() === "Fashion");
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 md:grid-cols-6 justify-between overflow-x-auto px-4 mx-auto w-full bg-white">
        {Array.isArray(Fashion) &&
          Fashion?.map((fashion, index) => (
            <div key={index} className="p-3 text-center flex items-center flex-col">
              <img
