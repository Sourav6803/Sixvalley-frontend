...
const FurniturePage = () => {
    ...
    useEffect(() => {
        const filterData = allProducts?.filter((product) => product?.category.trim() === "Furniture")
        setFilterProduct(filterData)
    }, [allProducts])

    useEffect(() => {
        const filterData = allSubSubCategory && allSubSubCategory?.filter(cat => cat.mainCategory.trim() === "Furniture")
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-5 md:grid-cols-6 justify-between overflow-x-auto px-4 mx-auto w-full bg-white">
                ...
                {Array.isArray(FurnitureData) && FurnitureData?.map((fashion, index) => (
